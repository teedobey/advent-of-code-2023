...
def main():
    ...
    for line in lines:
        ...
        springs = "?".join([springs for x in range(0, 5)])
    ...
    for i in range(0, max_iter + 1):
        if is_ok(next_num, i, springs):
            ...
            if len(springs[(i + next_num) :]) == 0 or springs[i + next_num] != "#":
                if (",".join([str(num) for num in numbers[1:]]), springs[(i + next_num + 1) :]) in memo:
                    score = memo[(",".join([str(num) for num in numbers[1:]]), springs[(i + next_num + 1) :])]
                else:
                    score = compute(
                        numbers[1:],
                    ...
                    memo[(",".join([str(num) for num in numbers[1:]]), springs[(i + next_num + 1) :])] = score
                total += score
